...
import { Record, MaintenanceReport, ReportStatus } from '../types';
...
interface DashboardProps {
  setView: (view: 'recordForm' | 'maintenanceForm' | 'recordList' | 'maintenanceList') => void;
  records: Record[];
...
export const Dashboard: React.FC<DashboardProps> = ({ setView, records, maintenanceReports }) => {
  ...
  const stats = useMemo(() => {
    return approvedRecords.reduce((acc, record) => {
      acc.totalWorshippers += record.tarawihWorshippers + record.qiyamWorshippers + record.fajrWorshippers;
      acc.iftarMeals += record.iftarMeals;
      acc.quranStudents += record.quranStudents;
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard title="طلاب الحلقات" value={stats.quranStudents.toLocaleString()} />
      ...
      <div className="bg-white p-6 rounded-lg shadow-md flex flex-col md:flex-row gap-4 items-center justify-center">
        ...
        <button onClick={() => setView('maintenanceForm')} className="w-full md:w-auto px-6 py-3 bg-secondary text-white font-bold rounded-lg hover:bg-opacity-90 transition">إضافة تقرير صيانة جديد</button>
      </div>
